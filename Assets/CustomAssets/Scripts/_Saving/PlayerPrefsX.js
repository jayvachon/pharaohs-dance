...
static function SetBoolArray (key : String, boolArray : boolean[]) {
	if (boolArray.Length == 0) return;
	var intArray : int[] = new int[boolArray.Length];
	for (var i : int = 0; i < boolArray.Length; i ++) {
		intArray[i] = boolArray [i] ? 1 : 0;
	}
	PlayerPrefsX.SetIntArray (key, intArray);
}

static function GetBoolArray (key : String) : boolean[] {
	if (PlayerPrefs.HasKey(key)) {
		var arr : int[] = GetIntArray (key);
		var boolArray : boolean[] = new boolean[arr.Length];
		for (var i = 0; i < arr.Length; i ++) {
			boolArray[i] = arr[i] == 1 ? true : false;
		}
		return boolArray;
	}
	return null;
